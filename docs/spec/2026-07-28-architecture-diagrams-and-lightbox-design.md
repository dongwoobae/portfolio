...
한 프로젝트의 스크린샷 전체가 하나의 갤러리다. `shotRows`를 평탄화한 뒤 `mobileShot`을 맨 뒤에 붙인 순서를 인덱스로 쓴다 — 화면에 그려지는 순서와 같아야 누른 것과 열리는 것이 어긋나지 않는다.

> **개정 (2026-07-29)** — 모바일 프레임은 원래 갤러리에서 빼 두었다. 근거는 "이미
> 실제 크기로 보인다"였는데 **사실이 아니었다.** 프레임 폭은 232px인데 촬영 논리
> 해상도는 390px이라 59%로 축소돼 있고, 세로로 긴 화면(영천중앙교회)은 47%까지
> 내려간다. 데스크톱 스크린샷과 똑같이 줄어 있으니 확대 경로도 똑같이 있어야 한다.
