...
```css
/* 이력서 인쇄 규격. 화면은 다크 터미널 테마 그대로 두고 종이에서만 반전한다.
   종이에서는 터미널 장식(프롬프트 접두사·커서·경로 표기·카드 보더)이 노이즈이므로
   전부 뺀다. 채용 담당자는 이 문서를 다른 지원자 이력서와 나란히 놓고 읽는다. */
@media print {
  @page {
    size: A4;
    margin: 15mm 18mm;
  ...
  :root {
    /* 화면 accent(#7ee2a8)는 민트라 흰 종이에서 대비가 부족해 읽히지 않는다.
       같은 색조를 유지하되 명도만 낮춘 인쇄 전용 값을 쓴다. */
    --print-accent: #1a7f4f;
  }
...
git commit -m "feat: 이력서 A4 1장 인쇄 스타일"
```
