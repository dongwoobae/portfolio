/* eslint-disable */
// Generated by Wrangler by running `wrangler types --env-interface=CloudflareEnv cloudflare-env.d.ts` (hash: 04d2b304e3942c128bbbcc2960e183d8)
// Runtime types generated with workerd@1.20260722.1 2026-07-02 global_fetch_strictly_public,nodejs_compat
interface __BaseEnv_CloudflareEnv {
	IMAGES: ImagesBinding;
	ASSETS: Fetcher;
	CLOUDFLARE_API_TOKEN: string;
	CLOUDFLARE_ACCESS_KEY_ID: string;
	CLOUDFLARE_SECRET_ACCESS_KEY_ID: string;
...
interface CloudflareEnv extends __BaseEnv_CloudflareEnv {
	// 아래 두 줄은 수동 선언이다. `wrangler types`는 secret을 알 수 없어
	// `npm run cf-typegen`을 다시 돌리면 지워진다. 재생성 후 diff에 이 두 줄이
	// 사라져 있으면 반드시 되살려라 (없으면 CI typecheck가 깨진다).
	// 값은 `wrangler secret put`(운영) / `.dev.vars`(로컬)로 주입한다.
...
declare namespace NodeJS {
	interface ProcessEnv extends StringifyValues<Pick<Cloudflare.Env, "CLOUDFLARE_API_TOKEN" | "CLOUDFLARE_ACCESS_KEY_ID" | "CLOUDFLARE_SECRET_ACCESS_KEY_ID" | "CLOUDFLARE_S3_API_ENDPOINT" | "CLOUDFLARE_ACCOUNT_ID" | "NEXT_PUBLIC_SITE_URL">> {}
}
